...
export interface Achievement {
  ...
  title: string;
  description: string;
  category: 'tasks' | 'focus' | 'mood' | 'streak' | 'level' | 'time' | 'special';
  tier: 'bronze' | 'silver' | 'gold' | 'platinum' | 'diamond' | 'legendary';
  animal: 'penguin' | 'bear' | 'cat' | 'dog' | 'rabbit' | 'fox' | 'owl' | 'lion' | 'tiger' | 'panda' | 'koala' | 'wolf' | 'deer' | 'elephant' | 'giraffe';
  condition: {
    type: 'tasks_completed' | 'focus_sessions' | 'mood_streak' | 'task_streak' | 'level_reached' | 'xp_earned' | 'login_streak' | 'time_based' | 'special';
    value: number;
...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  ...Array.from({ length: 50 }, (_, i) => ({
    id: `task_novice_${i + 1}`,
    title: `Task ${['Apprentice', 'Novice', 'Beginner', 'Starter', 'Rookie'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 30 }, (_, i) => ({
    id: `task_expert_${i + 1}`,
    title: `Task ${['Expert', 'Master', 'Champion', 'Legend', 'Grandmaster'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 20 }, (_, i) => ({
    ...
    title: `Task ${['Legendary', 'Mythical', 'Divine', 'Cosmic', 'Eternal'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 50 }, (_, i) => ({
    ...
    title: `Focus ${['Starter', 'Beginner', 'Learner', 'Student', 'Trainee'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 50 }, (_, i) => ({
    ...
    title: `Focus ${['Advanced', 'Expert', 'Master', 'Guru', 'Sage'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 50 }, (_, i) => ({
    ...
    title: `Focus ${['Grandmaster', 'Legend', 'Titan', 'God', 'Supreme'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 50 }, (_, i) => ({
    id: `task_streak_${i + 1}`,
    title: `${['Consistent', 'Reliable', 'Steady', 'Persistent', 'Dedicated'][i % 5]} ${['Cat', 'Dog', 'Rabbit', 'Fox', 'Penguin'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 30 }, (_, i) => ({
    ...
    title: `${['Focused', 'Concentrated', 'Mindful', 'Zen', 'Meditative'][i % 5]} ${['Bear', 'Owl', 'Lion', 'Tiger', 'Wolf'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 40 }, (_, i) => ({
    id: `mood_streak_${i + 1}`,
    title: `${['Happy', 'Joyful', 'Cheerful', 'Positive', 'Radiant'][i % 5]} ${['Panda', 'Koala', 'Elephant', 'Giraffe', 'Deer'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 100 }, (_, i) => ({
    id: `level_${i + 1}`,
    title: `Level ${i + 1} ${['Warrior', 'Knight', 'Champion', 'Hero', 'Legend', 'Master', 'Sage', 'Guardian', 'Titan', 'God'][i % 10]}`,
    description: `Reach level ${i + 1}`,
    category: 'level' as const,
    tier: i < 20 ? 'bronze' : i < 50 ? 'silver' : i < 80 ? 'gold' : i < 95 ? 'platinum' : 'legendary' as const,
    animal: ['cat', 'dog', 'rabbit', 'fox', 'penguin', 'bear', 'owl', 'lion', 'tiger', 'wolf', 'panda', 'koala', 'elephant', 'giraffe', 'deer'][i % 15] as const,
    condition: {
  ...
  ...Array.from({ length: 100 }, (_, i) => ({
    id: `xp_${i + 1}`,
    title: `${['Collector', 'Gatherer', 'Accumulator', 'Hoarder', 'Magnate'][i % 5]} ${['Cat', 'Dog', 'Rabbit', 'Fox', 'Penguin', 'Bear', 'Owl', 'Lion', 'Tiger', 'Wolf'][i % 10]} ${Math.floor(i / 10) + 1}`,
    description: `Earn ${(i + 1) * 1000} total XP`,
    category: 'level' as const,
    tier: i < 25 ? 'bronze' : i < 50 ? 'silver' : i < 75 ? 'gold' : i < 90 ? 'platinum' : 'legendary' as const,
    animal: ['cat', 'dog', 'rabbit', 'fox', 'penguin', 'bear', 'owl', 'lion', 'tiger', 'wolf', 'panda', 'koala', 'elephant', 'giraffe', 'deer'][i % 15] as const,
    condition: {
  ...
  ...Array.from({ length: 30 }, (_, i) => ({
    id: `daily_${i + 1}`,
    title: `Daily ${['Visitor', 'Regular', 'Devotee', 'Fanatic', 'Addict'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 20 }, (_, i) => ({
    id: `weekly_${i + 1}`,
    title: `Weekly ${['Champion', 'Master', 'Legend', 'Titan', 'God'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 12 }, (_, i) => ({
    id: `monthly_${i + 1}`,
    title: `Monthly ${['Achiever', 'Conqueror', 'Dominator', 'Emperor'][i % 4]} ${Math.floor(i / 4) + 1}`,
  ...
  ...Array.from({ length: 50 }, (_, i) => ({
    ...
    title: `${['Combo', 'Multi', 'Super', 'Ultra', 'Mega'][i % 5]} ${['Cat', 'Dog', 'Rabbit', 'Fox', 'Penguin', 'Bear', 'Owl', 'Lion', 'Tiger', 'Wolf'][i % 10]} ${Math.floor(i / 10) + 1}`,
  ...
  ...Array.from({ length: 25 }, (_, i) => ({
    ...
    title: `Perfect ${['Week', 'Fortnight', 'Month', 'Season', 'Year'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 25 }, (_, i) => ({
    id: `speed_demon_${i + 1}`,
    title: `Speed ${['Demon', 'Racer', 'Lightning', 'Flash', 'Sonic'][i % 5]} ${Math.floor(i / 5) + 1}`,
  ...
  ...Array.from({ length: 12 }, (_, i) => ({
    id: `seasonal_${i + 1}`,
    title: `${['Spring', 'Summer', 'Autumn', 'Winter'][i % 4]} ${['Blossom', 'Sunshine', 'Harvest', 'Frost'][i % 4]} ${Math.floor(i / 4) + 1}`,
    description: `Complete seasonal challenges during ${['Spring', 'Summer', 'Autumn', 'Winter'][i % 4]}`,
    category: 'special' as const,
    tier: 'gold' as const,
